...
PUB = ROOT / "public"
V = "20260924b"  # cache-bust for css/js
...
def build_home():
  ...
  {people_grid()}
    ...
    body += f'''<section class="section-tight" style="padding-bottom:0"><div class="wrap" style="text-align:center;margin-bottom:26px"><div class="eyebrow reveal" style="justify-content:center">Where we work</div><h3 class="reveal">Zoning matters in {JCOUNT} jurisdictions across Ohio and Northern Kentucky</h3><div class="reveal" data-delay="1">{link("/jurisdictions/", "See the list")}</div></div>{marquee()}</section>'''
    body += cta()
...
def build_practices():
  ...
  <div class="item"><b>{JCOUNT}</b><span>jurisdictions across Ohio and Northern Kentucky</span></div>
...
def build_jurisdictions():
    ...
    body = f'''<section class="page-head"><div class="wrap"><div class="eyebrow">Jurisdictions</div><h1>{JCOUNT} jurisdictions. One firm.</h1><p class="lede">We have represented clients on zoning matters in {JCOUNT} jurisdictions in Ohio and Northern Kentucky, from Cincinnati's neighborhoods to Cleveland, Columbus, Toledo, and Akron.</p></div></section>
{marquee()}
<section><div class="wrap"><ul class="list-cols">{lis}</ul><p class="muted" style="margin-top:28px;font-size:14px">A representative list. We have appeared in others not named here.</p></div></section>''' + cta("Your project is in a jurisdiction we know.", "Boards, staff, codes, and neighbors: we have probably been in that hearing room before.")
    write("/jurisdictions/", page("Jurisdictions", f"Suder, LLC has handled zoning matters in {JCOUNT} jurisdictions across Ohio and Northern Kentucky.", body, "/jurisdictions/", dark_hero=False))
